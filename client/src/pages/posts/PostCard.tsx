			...
			{postData?.slice(0, length).map((item: ItemType) => {
				return (
					<a key={item.id} href={`/@${item.user.username}/${item.slug}`} className='my-5'>
						<div>
							{item.featuredImage_url && <img src={getImageUrl(item.featuredImage_url)} alt="" className='h-72 w-full object-cover' />}
							<div className='flex flex-col justify-between h-48'>
								...
								<div className='flex flex-row h-14 items-center'>
									{item.user.avatar_url && <img src={getAvatarUrl(item.user.avatar_url)} alt="" className="w-12 h-12 rounded-full mr-3" />}
									<div>
